...
Aout="-o $scriptPath/output/slotted_aloha.out"
catFile="$scriptPath/catstat.csv" #compiled stats
i=0

for OFFERED_LOAD in $load_seq
do
	$( python $scriptPath/generator.py $NUM_PKTS_PER_NODE  $NUM_NODE $PKT_SIZE -l $OFFERED_LOAD $OUTFILE$i && python $scriptPath/slotted_aloha.py $Atraffic$i $Aout$i ) &
  let "i++"
done
